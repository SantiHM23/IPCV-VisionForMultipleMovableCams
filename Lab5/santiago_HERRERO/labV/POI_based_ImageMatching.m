...
% close all
% clc
function [matchedPoints, points, features, MaxRatio, Metric] = POI_based_ImageMatching(filename, dtctr, dscrptr)
%% PARAMETER DEFINITION %%

%%define dataset path
params.Directory    = fullfile(filename);

%%Detector parametersk
params.detector     =  dtctr; %'LoG', 'HoG', 'KDetector', 'SURF', 'SIFT', ...
params.nscales      =        5;
params.noctaves     =        3;
...
params.descriptor   =  dscrptr; % 'SIFT', 'SURF', 'DSP-SIFT', 'KAZE', ...
params.desOnDecom   =    false; % describe on scale-space (linear or non-linear) decomposition (if available). False is NonLinear
