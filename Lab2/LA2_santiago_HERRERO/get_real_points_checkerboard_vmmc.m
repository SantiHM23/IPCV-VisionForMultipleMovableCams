...
% GET_REAL_POINTS_CHECKERBOARD_IAVS(Np, widhtP, display)  returns a list of Np points 
% automatically selected for a checkerboard pattern with real dimensions 
% defined by the witdh of each square in the checkerboard pattern (widhtP)
%
...
step = (sqrt(Np)-1)/2;
m = 1;
for j = -widhtP/2 : widhtP/(2*step) : widhtP/2
    for i = -widhtP/2 : widhtP/(2*step) : widhtP/2
...
if display == 1
    ...
    x = coords(:,1) + widhtP/2; x(x==0)=1;
    y = coords(:,2) - widhtP/2; y=abs(y);y(y==0)=1;
        
    ima_pattern=checkerboard(widhtP/8) > 0.5;
